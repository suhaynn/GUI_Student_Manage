...
class LoginGUI:
    def __init__(self, root):
        ...
        tk.Radiobutton(self.root, text="管理员", variable=self.user_type, value=2).grid(row=2, column=3, padx=10,pady=5)
    ...
    def login(self):
        ...
        if query:
            print(query)
            # query[0][3]表示第一条记录的第四个字段的值，也就是用户类型（0表示学生，1表示老师，2表示管理员）。
            # 数据库下标是从0开始
            user_type = query[0][3]
            if user_type == 0:
